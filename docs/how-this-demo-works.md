...
Both pages use values from `frontmatter/thesis_metadata.tex`. For example, `\newcommand{\StudentName}{Sooraj Sudhakaran}` defines a reusable value. The demonstration shows `XXX` for the matriculation number, second supervisor and submission date. Replace and verify these values in one place before submission. The general example title is “Automatisierte Programmerzeugung für Robotersysteme” in German and “Automated Program Generation for Robotic Systems” in English. Set `\ThesisTitleGerman` and `\ThesisTitle` in the metadata file; the German title appears above the English title on the cover and inner title page. These illustrate bilingual title formatting. The short sand-timer examples in the chapters remain separate fictional teaching material, not research supporting the robotics title. The layout demonstrates the inspected working project, but students must compare it with their current programme-specific approved title form.
